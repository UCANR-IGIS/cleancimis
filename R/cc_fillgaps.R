...
cc_fillgaps <- function(stations,
  ...
  for (i in 1:length(stations)) {
    ...
    message(crayon::green(paste0(" - start time: ", format(Sys.time(), "%H:%M"))))
    ...
    gap_fill_stns <- setNames(c(stid_this, stid_nn), LETTERS[1:(length(stid_nn) + 1)])
    ...
    gapfilled_rds <- file.path(gapfilldata_dir, paste0(stids_years_chr, "_gapfilled.rds"))
    if (save_forest) {
    ...
    if (all_files_found_yn && !overwrite) {
      ...
      message(crayon::green(paste0(" - found saved gap filled data for #", stid_this, ", skipping")))
      res$gapfilled <- c(res$gapfilled, gapfilled_rds)

      if (save_forest) {
        message(crayon::green(paste0(" - found saved forests for #", stid_this, ", skipping")))
      ...
      ## First, we:
      ## 1. import the rds files containing the gappy data aggregated by calendar year (i.e., files are output by cc_csv2rds)
      ...
      ## Check to make sure the gappy data has all the stations needed
      if (FALSE %in% (gap_fill_stns %in% unique(years_comb_tbl$station_n))) {
        warning(paste0("Can not gap fill data for station ", stid_this, ". Missing gappy data for: ",
                       paste(gap_fill_stns[!gap_fill_stns %in% unique(years_comb_tbl$station_n)], collapse = ", "), "."))

      } else {
        ## Filter the combined gappy data for only rows for the selection stations, drop some columns, and create some new columns
        hourly_tbl <- years_comb_tbl |>
          ## bind_rows() |>       I'm not sure why this was here, it has no effect.
        ...
        message(crayon::silver(" - TODO: Check that the date range for the 4 stations is the same (no big gaps)"))
        ...
        if (save_forest) {
          ...
          stop("I have disabled save_forests for now because the files are huge (>2GB). See TODO.md for details about next steps.")
          ...
          ## It appears to hang right here
          message(crayon::green(paste0(" - going to save the forest: ", basename(forests_rds))))
          ...
          ## xz is the best compression (by far)
          saveRDS(hourly_tbl, gapfilled_rds, compress = "xz")
        ...
        cat("RIGHT HERE TRY TO EXTRACT JUST THE ORIGINAL FIELDS FOR THE MAIN STATION \n")
        ...
        #########################################
        ## DONE WITH PROCESSING THIS STATION


      } ## if all stations are available in the gappy data
